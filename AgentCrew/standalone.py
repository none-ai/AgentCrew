...
class AgentCrewRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == "/teams":
            self._send_json(HTTPStatus.OK, {"teams": self.app.list_teams()})
            return

        if parsed.path == "/tasks":
            self._send_json(HTTPStatus.OK, {"tasks": self.app.list_tasks()})
            return

        if len(parts) == 2 and parts[0] == "tasks":
            task = self.app.get_task(parts[1])
            if not task:
        ...
        if parsed.path == "/messages":
            ...
            if not agent:
                ...
                return
            self._send_json(HTTPStatus.OK, {"messages": self.app.get_inbox(agent)})
            return
