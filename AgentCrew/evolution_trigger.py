#!/usr/bin/env python3
"""
AgentCrew 自我进化定时触发器
定时执行自我巡检和迭代
"""
import os
import sys
import time
import json
import subprocess
import signal
from datetime import datetime
from pathlib import Path

try:
    from .runtime import get_runtime_paths, load_json_config, resolve_workspace_path
except ImportError:
    from runtime import get_runtime_paths, load_json_config, resolve_workspace_path

RUNTIME = get_runtime_paths()
WORKSPACE = str(RUNTIME.workspace)
AGENTCREW_DIR = str(RUNTIME.package_dir)
LOG_DIR = str(RUNTIME.logs_dir)
PID_FILE = str(RUNTIME.tmp_dir / "agentcrew_evolution.pid")
LOG_FILE = str(RUNTIME.logs_dir / "evolution_trigger.log")

# 进化间隔 (秒)
DEFAULT_INTERVAL = 3600  # 1小时
MIN_INTERVAL = 300       # 最小5分钟


def log(msg: str):
    """日志输出"""
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    line = f"[{timestamp}] {msg}"
    print(line)
    os.makedirs(LOG_DIR, exist_ok=True)
    with open(LOG_FILE, "a") as f:
        f.write(line + "\n")


def load_config() -> dict:
    """加载配置"""
    default_config = {
        "interval_seconds": DEFAULT_INTERVAL,
        "auto_fix": False,
        "target_dirs": [
            f"{WORKSPACE}/scripts",
            f"{WORKSPACE}/AgentCrew/AgentCrew"
        ],
        "enabled": True,
        "notification": False
    }
    config = load_json_config("evolution_config.json", default_config, WORKSPACE)
    config["target_dirs"] = [
        str(resolve_workspace_path(target, WORKSPACE))
        for target in config.get("target_dirs", [])
    ]
    return config


def save_config(config: dict):
    """保存配置"""
    config_file = Path(AGENTCREW_DIR) / "evolution_config.json"
    with open(config_file, 'w', encoding="utf-8") as f:
        json.dump(config, f, indent=2)


def write_pid():
    """写入 PID"""
    with open(PID_FILE, 'w') as f:
        f.write(str(os.getpid()))


def remove_pid():
    """删除 PID 文件"""
    if os.path.exists(PID_FILE):
        os.remove(PID_FILE)


def signal_handler(signum, frame):
    """信号处理"""
    log(f"📡 收到信号 {signum}, 准备停止...")
    remove_pid()
    sys.exit(0)


def run_evolution(target_dirs: list, auto_fix: bool) -> dict:
    """运行进化周期"""
    log("🔄 开始进化周期...")
    
    # 构建命令
    cmd = [
        sys.executable,
        f"{AGENTCREW_DIR}/self_evolution.py"
    ]
    
    if auto_fix:
        cmd.append("--auto-fix")
    
    # 添加目标目录
    for d in target_dirs:
        cmd.extend(["--target", d])
    
    # 添加模拟运行标志
    if not auto_fix:
        cmd.append("--no-dry-run")
    
    try:
        result = subprocess.run(
            cmd,
            capture_output=True,
            text=True,
            timeout=600,  # 10分钟超时
            cwd=AGENTCREW_DIR
        )
        
        if result.returncode == 0:
            log("✅ 进化周期完成")
            # 尝试解析输出中的统计数据
            try:
                output = result.stdout
                if "issues_found" in output:
                    # 输出包含 JSON
                    log(f"📊 {output.split('{}')[1] if '{}' in output else '完成'}")
            except Exception:
                pass
            return {"status": "success", "output": result.stdout}
        else:
            log(f"❌ 进化周期失败: {result.stderr}")
            return {"status": "error", "error": result.stderr}
            
    except subprocess.TimeoutExpired:
        log("⏱️ 进化周期超时")
        return {"status": "timeout", "error": "执行超时"}
    except Exception as e:
        log(f"❌ 进化周期异常: {e}")
        return {"status": "error", "error": str(e)}


def check_running() -> bool:
    """检查是否已有实例在运行"""
    if not os.path.exists(PID_FILE):
        return False
    
    try:
        with open(PID_FILE, 'r') as f:
            pid = int(f.read().strip())
        
        # 检查进程是否存在
        os.kill(pid, 0)
        return True
    except Exception:
        # PID 文件存在但进程不在，删除它
        remove_pid()
        return False


def start_daemon(interval: int = None):
    """启动守护进程"""
    if check_running():
        log("⚠️ 已有实例在运行")
        return
    
    config = load_config()
    if interval:
        config["interval_seconds"] = interval
    
    write_pid()
    signal.signal(signal.SIGINT, signal_handler)
    signal.signal(signal.SIGTERM, signal_handler)
    
    log(f"🚀 AgentCrew 自我进化守护进程已启动")
    log(f"   间隔: {config['interval_seconds']} 秒")
    log(f"   自动修复: {config['auto_fix']}")
    log(f"   目标: {config['target_dirs']}")
    
    # 立即运行一次
    run_evolution(config["target_dirs"], config["auto_fix"])
    
    while True:
        time.sleep(config["interval_seconds"])
        
        # 重新加载配置（支持热更新）
        config = load_config()
        
        if config.get("enabled", True):
            run_evolution(config["target_dirs"], config["auto_fix"])
        else:
            log("⏸️ 进化已禁用")


def stop_daemon():
    """停止守护进程"""
    if check_running():
        try:
            with open(PID_FILE, 'r') as f:
                pid = int(f.read().strip())
            os.kill(pid, signal.SIGTERM)
            log(f"📴 已发送停止信号到 {pid}")
            remove_pid()
        except Exception as e:
            log(f"❌ 停止失败: {e}")
    else:
        log("⚠️ 没有运行的守护进程")


def status():
    """查看状态"""
    if check_running():
        with open(PID_FILE, 'r') as f:
            pid = f.read().strip()
        
        config = load_config()
        print(f"✅ 守护进程运行中 (PID: {pid})")
        print(f"   间隔: {config['interval_seconds']} 秒")
        print(f"   自动修复: {config['auto_fix']}")
    else:
        print("❌ 守护进程未运行")


def main():
    import argparse
    
    parser = argparse.ArgumentParser(description="AgentCrew 自我进化触发器")
    parser.add_argument("action", nargs="?", choices=["start", "stop", "restart", "status", "run"], 
                       help="操作: start/stop/restart/status/run")
    parser.add_argument("--interval", "-i", type=int, help="间隔秒数")
    parser.add_argument("--auto-fix", "-a", action="store_true", help="启用自动修复")
    parser.add_argument("--once", "-o", action="store_true", help="仅运行一次")
    
    args = parser.parse_args()
    action = args.action or "run"
    
    if action == "start":
        start_daemon(args.interval)
    elif action == "stop":
        stop_daemon()
    elif action == "restart":
        stop_daemon()
        time.sleep(2)
        start_daemon(args.interval)
    elif action == "status":
        status()
    elif action == "run":
        config = load_config()
        if args.auto_fix:
            config["auto_fix"] = True
        run_evolution(config["target_dirs"], config["auto_fix"])


if __name__ == "__main__":
    main()
